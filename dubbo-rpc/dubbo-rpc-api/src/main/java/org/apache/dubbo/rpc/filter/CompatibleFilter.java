...
public class CompatibleFilter implements Filter {
    ...
    private static final String  JSON_SERIALIZATION="json";
        ...
        if (!invocation.getMethodName().startsWith("$") && !result.hasException()) {
            ...
            if (value != null) {
                try {
                    ...
                    if (JSON_SERIALIZATION.equals(serialization) || FASTJSON_SERIALIZATION.equals(serialization)) {
                        // If the serialization key is json or fastjson
                        Type returnType = method.getGenericReturnType();
                        newValue = PojoUtils.realize(value, type, returnType);
